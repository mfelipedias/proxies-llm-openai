# 🚧 Kimi Proxy (kimi.com / Moonshot AI) — porta 3004
...
e exporá kimi.com via API OpenAI-compatible em `http://localhost:3004/v1`.

## O que o stub atual faz

```bash
npm install
npm start          # sobe em :3004
curl http://localhost:3004/health   # → { "status": "scaffold", ... }
```
...
1. Leia o [PLAN.md](PLAN.md) — a **Fase 1 (reconhecimento)** decide tudo: se o kimi.com
   aceita API direta com token (caminho qwen, mais simples) ou exige bridge de UI
   (caminho glm/minimax).
