 ...
 * Supports JSON, Hermes-style XML <parameter> and Kimi3-Coder
...
// - <tool_call> ......... </tool_call>          (formato injetado pelo nosso prompt; também o Hermes/Kimi3)
...
// - <function=x> ........ </function>           (formato NATIVO do Kimi3-Coder, quando emitido sem o wrapper)
...
function extractToolName(openTag: string, block: string): string {
  ...
  // Formato Kimi3-Coder: <function=read_file>
  const kimiFnMatch = combined.match(/<function=["']?([^"'>\s]+)["']?\s*>/i);
  if (kimiFnMatch) return kimiFnMatch[1];
...
//   <parameter=path>value</parameter>          (nativo Kimi3-Coder)
 ...
 * (and the Kimi3-Coder <parameter=key>value</parameter> variant).
 */
