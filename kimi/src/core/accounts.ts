...
export function loadAccounts(): KimiAccount[] {
  const db = getDatabase()
  const rows = db.prepare('SELECT id, email, password FROM accounts ORDER BY created_at ASC').all()
  return rows as KimiAccount[]
}

export function addAccount(email: string, password: string, id?: string): KimiAccount {
...
export function listAccounts(): KimiAccount[] {
  return loadAccounts().map(a => ({ id: a.id, email: a.email, password: '***' }))
}

export function getAccountCredentials(id: string): KimiAccount | undefined {
  const db = getDatabase()
  const row = db.prepare('SELECT id, email, password FROM accounts WHERE id = ?').get(id)
  return row as KimiAccount | undefined
}
