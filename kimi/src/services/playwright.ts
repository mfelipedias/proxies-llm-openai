 ...
 *
 * ⚠️ SCAFFOLD (PLAN Fase 3) — molde herdado do qwen. As URLs de login/sessão
 * `*.kimi.ai` e os seletores são PLACEHOLDERS do qwen. Mapear o fluxo real do
 * kimi.com na recon (PLAN Fase 1) e ajustar antes de usar.
 */
...
import { KimiAccount } from '../core/accounts.ts';
...
/** Sessão Kimi logada? Sinal forte: cookie `token` não vazio em chat.kimi.ai. */
function hasKimiSessionCookies(cookies: Array<{ name: string; value: string }>): boolean {
...
interface AccountHeaderCache {
  ...
  cachedKimiHeaders: { headers: Record<string, string>, chatSessionId: string, parentMessageId: string | null } | null;
...
export async function initPlaywright(headless = true, browserType: BrowserType = 'chromium') {
  ...
  const profilePath = path.resolve('kimi_profiles', '_default');
  ...
  const hasCredentials = !!(process.env.KIMI_EMAIL && process.env.KIMI_PASSWORD);
...
async function checkValidSession(): Promise<boolean> {
  ...
  try {
    ...
    const hasAuthCookie = hasKimiSessionCookies(cookies);
    if (!hasAuthCookie) return false;
    await activePage.goto('https://chat.kimi.ai/', { waitUntil: 'domcontentloaded', timeout: 10000 });
...
export async function loginToKimi(email: string, password: string): Promise<boolean> {
  if (!activePage) throw new Error('Playwright not initialized');

  console.log(`[Playwright] Attempting API login for ${email}...`);
  
  await activePage.goto('https://chat.kimi.ai/auth', { waitUntil: 'domcontentloaded' });

  const hashedPassword = crypto.createHash('sha256').update(password).digest('hex');

  const result = await activePage.evaluate(async ({ email, password }) => {
    try {
      const response = await fetch("https://chat.kimi.ai/api/v2/auths/signin", {
        method: "POST",
        headers: {
  ...
  if (result.ok) {
    ...
    await activePage.goto('https://chat.kimi.ai/', { waitUntil: 'domcontentloaded' });
    const isLogged = !(activePage.url().includes('auth') || activePage.url().includes('login'));
    if (isLogged) {
...
async function loginToKimiUI(email: string, password: string): Promise<boolean> {
  if (!activePage) throw new Error('Playwright not initialized');
...
export async function getKimiHeaders(forceNew = false, accountId?: string): Promise<{ headers: Record<string, string>, chatSessionId: string, parentMessageId: string | null }> {
  const cacheKey = accountId || 'global';
  const cache = getAccountHeaderCache(cacheKey);

  if (!forceNew && cache.cachedKimiHeaders && (Date.now() - cache.lastHeadersTime < HEADERS_TTL * REFRESH_THRESHOLD)) {
    return cache.cachedKimiHeaders;
  }
  const release = await uiMutex.acquire();
  try {
    if (!forceNew && cache.cachedKimiHeaders && (Date.now() - cache.lastHeadersTime < HEADERS_TTL)) {
      release();
      return cache.cachedKimiHeaders;
    }
    return await _getKimiHeadersInternal(forceNew, accountId);
  } finally {
    release();
  }
}

async function _getKimiHeadersInternal(forceNew = false, accountId?: string): Promise<{ headers: Record<string, string>, chatSessionId: string, parentMessageId: string | null }> {
  const cacheKey = accountId || 'global';
  const cache = getAccountHeaderCache(cacheKey);
  ...
  if (!forceNew && cache.cachedKimiHeaders && (Date.now() - cache.lastHeadersTime < HEADERS_TTL)) {
    ...
    if (age > HEADERS_TTL * REFRESH_THRESHOLD && !cache.refreshTimeout) {
      cache.refreshTimeout = setTimeout(() => {
        cache.refreshTimeout = null;
        getKimiHeaders(true, accountId).catch(() => {});
      }, HEADERS_TTL - age);
    }
    return cache.cachedKimiHeaders;
  }
  ...
  const isOnKimi = currentUrl.includes('chat.kimi.ai');
  const isOnSpecificChat = isOnKimi && /\/c\//.test(currentUrl);

  if (!isOnKimi || forceNew || isOnSpecificChat) {
    console.log(`[Playwright] Navigating to Kimi home for ${cacheKey}... (Current: ${currentUrl})`);
    try {
      await page.goto('https://chat.kimi.ai/', { waitUntil: 'domcontentloaded', timeout: 20000 });
    } catch (navErr: any) {
      ...
      if (accountId) {
        ...
        await fresh.goto('https://chat.kimi.ai/', { waitUntil: 'domcontentloaded', timeout: 20000 });
      } else {
        throw navErr;
      }
    }
  ...
  if (isLoginPage) {
    if (!accountId) {
      ...
      if (email && password) {
        ...
        try {
          const loggedIn = await loginToKimi(email, password);
          if (!loggedIn) {
            throw new Error('loginToKimi returned false');
          }
        ...
        console.warn('[Playwright] Detected login page but KIMI_EMAIL/PASSWORD not provided in .env');
      }
    } else {
      const { getAccountCredentials } = await import('../core/accounts.ts');
      const creds = getAccountCredentials(accountId);
      if (creds && creds.email && creds.password) {
        ...
        if (acctContext) {
          await loginToKimiWithContext(acctContext, page, creds.email, creds.password);
        }
      }
    }
  }
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(async () => {
      console.error(`[Playwright] Timeout waiting for Kimi headers for ${cacheKey}. Current URL:`, page.url());
      try {
        const screenshotPath = path.resolve(`kimi_profiles/error_${cacheKey}.png`);
      ...
      reject(new Error(`Timeout waiting for Kimi headers for ${cacheKey}`));
    }, 60000);
    ...
    const routeHandler = async (route: any, request: any) => {
      ...
      cache.cachedKimiHeaders = { headers: extractedHeaders, chatSessionId: uiSessionId, parentMessageId: uiParentMessageId };
      ...
      // (molde qwen) desligar tools nativas — não se aplica ao kimi; ver Fase 3.

      await route.abort('aborted');
      
      await page.unroute('**/api/v2/chat/completions*', routeHandler);

      resolve(cache.cachedKimiHeaders);
    };
...
export async function initPlaywrightForAccount(account: KimiAccount, headless = true, browserType: BrowserType = 'chromium') {
  const profilePath = path.resolve('kimi_profiles', account.id);
  ...
  if (!hasAuthCookie && account.email && account.password) {
    await loginToKimiWithContext(acctContext, acctPage, account.email, account.password);
  }
}
...
export async function launchManualLoginAccount(accountId: string, browserType: BrowserType = 'chromium'): Promise<{ context: BrowserContext, page: Page }> {
  const profilePath = path.resolve('kimi_profiles', accountId);
  ...
  const acctPage = await acctContext.newPage();
  await acctPage.goto('https://chat.kimi.ai/auth', { waitUntil: 'domcontentloaded' });
...
export async function extractAccountInfoFromContext(page: Page): Promise<{ email: string | null, hasSession: boolean }> {
  const cookies = await page.context().cookies();
  const hasSession = hasKimiSessionCookies(cookies);
 ...
 * watchdog de stall do stream (kimi.ts): headers/bx-ua podem ter expirado e a
...
export async function getBrowserHealth(): Promise<{ browser: 'up' | 'down'; loggedIn: boolean | 'unknown'; accounts: Record<string, { browser: 'up' | 'down'; loggedIn: boolean | 'unknown' }> }> {
  ...
  const checkPage = async (page: Page | null | undefined): Promise<{ browser: 'up' | 'down'; loggedIn: boolean | 'unknown' }> => {
    ...
    try {
      const cookies = await page.context().cookies();
      return { browser: 'up', loggedIn: hasKimiSessionCookies(cookies) };
    } catch {
...
async function loginToKimiWithContext(acctContext: BrowserContext, acctPage: Page, email: string, password: string): Promise<boolean> {
  await acctPage.goto('https://chat.kimi.ai/auth', { waitUntil: 'domcontentloaded' });
  ...
  if (result.ok) {
    await acctPage.goto('https://chat.kimi.ai/', { waitUntil: 'domcontentloaded' });
