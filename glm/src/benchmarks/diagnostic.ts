...
async function measurePlaywrightInit() {
  ...
  const { initPlaywright, getGLMAuth } = await import('../services/playwright.ts');
...
async function measureNetworkLatency() {
  ...
  const start = performance.now();
  await fetch(`${config.glm.baseUrl}/api/models`, {
...
async function runDiagnostic() {
  ...
  try {
    ...
    console.log(`\nNetwork latency to GLM (z.ai): ${net.networkLatencyMs}ms`);
  } catch (e: any) {
    console.log(`Network: SKIP (${e.message})`);
  }
  
  console.log('\n=== Likely Bottlenecks ===');
  console.log('1. getGLMAuth() UI interactions: 2000-5000ms per call');
  console.log('2. Global mutex (glmChatMutex): serializes all chat requests');
  console.log('3. No header caching between requests: re-fetches token each time');
