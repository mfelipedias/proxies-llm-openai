...
async function runBenchmarks() {
  console.log('=== GLMProxy Speed Benchmark ===\n');
  ...
  results.push(await benchmarkOperation(
    'getGLMAuth (cached)',
    async () => {
      const { getGLMAuth } = await import('../services/playwright.ts');
      await getGLMAuth();
    },
    5
  ));
  ...
  results.push(await benchmarkOperation(
    'fetchGLMModels',
    async () => {
      const { fetchGLMModels } = await import('../services/glm.ts');
      await fetchGLMModels();
    },
    3
  ));
