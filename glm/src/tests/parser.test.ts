...
test('StreamingToolParser: GLM-5.1 <tool_call_block> wrapper', () => {
  const parser = new StreamingToolParser(READ_TOOL as any);
...
test('StreamingToolParser: bare JSON tool call (no tags) resolved as soon as it closes', () => {
  const parser = new StreamingToolParser(READ_TOOL as any);
  // GLM-5.1 às vezes emite JSON cru com lixo no início e sem o wrapper <tool_call>.
  // Assim que o JSON fecha, o tool call é emitido no próprio feed (sem esperar o flush).
  const r1 = parser.feed('④\n{"name": "read_file", "arguments": {"path": "package.json"}}\n');
  const flushed = parser.flush();
  const calls = [...r1.toolCalls, ...flushed.toolCalls];
  assert.strictEqual(calls.length, 1);
  assert.strictEqual(calls[0].name, 'read_file');
  assert.strictEqual(calls[0].arguments.path, 'package.json');
});
...
test('StreamingToolParser: bare JSON split across chunks (open brace at boundary)', () => {
  ...
  const calls = [
    ...parser.feed('EREAD_FILE({').toolCalls,
    ...parser.feed('"name": "read_file", "arguments": {"path": ').toolCalls,
    ...parser.feed('"package.json"}})').toolCalls,
    ...parser.flush().toolCalls,
  ];
  assert.strictEqual(calls.length, 1);
  assert.strictEqual(calls[0].name, 'read_file');
  assert.strictEqual(calls[0].arguments.path, 'package.json');
...
test('StreamingToolParser: bare JSON with UNKNOWN tool name stays as text', () => {
  const parser = new StreamingToolParser(READ_TOOL as any);
  const r = parser.feed('{"name": "not_a_real_tool", "arguments": {"x": 1}}');
  const flushed = parser.flush();
  assert.strictEqual(r.toolCalls.length + flushed.toolCalls.length, 0, 'no false positive');
  assert.ok((r.text + flushed.text).includes('not_a_real_tool'), 'unrecognized JSON returned as text');
});

test('StreamingToolParser: stray open brace in prose does not freeze streaming', () => {
  const parser = new StreamingToolParser(READ_TOOL as any);
  // `{` sem `}` em prosa/código: depois da janela curta, o texto continua fluindo.
  let text = parser.feed('o bloco começa com { e segue: ').text;
  text += parser.feed('mais texto que precisa chegar em tempo real ao cliente').text;
  assert.ok(text.includes('mais texto que precisa chegar'), 'text after stray { must stream');
  const flushed = parser.flush();
  assert.strictEqual(flushed.toolCalls.length, 0);
});

test('StreamingToolParser: bare JSON inside code fence is treated as example, not tool call', () => {
  const parser = new StreamingToolParser(READ_TOOL as any);
  const r = parser.feed('Use assim:\n```json\n{"name": "read_file", "arguments": {"path": "x"}}\n```\nFim.');
  const flushed = parser.flush();
  assert.strictEqual(r.toolCalls.length + flushed.toolCalls.length, 0, 'fenced JSON is documentation');
  assert.ok((r.text + flushed.text).includes('read_file'), 'fenced JSON stays as text');
});

test('StreamingToolParser: unclosed bare JSON at end of stream is repaired at flush', () => {
  const parser = new StreamingToolParser(READ_TOOL as any);
  parser.feed('{"name": "read_file", "arguments": {"path": "package.json"');
  const flushed = parser.flush();
  assert.strictEqual(flushed.toolCalls.length, 1);
