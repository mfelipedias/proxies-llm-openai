import { GLMAccount, loadAccounts } from './accounts.ts'
...
// Conta sintética usada no modo "zero-config" (sem contas no DB): roteia para a
// página global, que carrega a sessão de glm_session.json (real) ou um token de
// convidado. Permite `docker compose up` funcionar sem cadastrar conta.
const GLOBAL_ACCOUNT: GLMAccount = { id: 'global', email: 'global', password: '' }

export function getNextAccount(): GLMAccount | null {
...
export function getNextAvailableAccount(skipAccountId?: string): GLMAccount | null {
  const accounts = loadAccounts()
  if (accounts.length === 0) {
    // Zero-config: só existe a conta global. Se já foi tentada, acabou.
    return skipAccountId === 'global' ? null : GLOBAL_ACCOUNT
  }
