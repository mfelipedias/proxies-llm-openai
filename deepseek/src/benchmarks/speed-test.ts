...
async function runBenchmarks() {
  console.log('=== DeepSeekProxy Speed Benchmark ===\n');
  ...
  results.push(await benchmarkOperation(
    'getDeepSeekAuth',
    async () => {
      const { getDeepSeekAuth } = await import('../services/playwright.ts');
  ...
  results.push(await benchmarkOperation(
    'fetchDeepSeekModels',
    async () => {
      const { fetchDeepSeekModels } = await import('../services/deepseek.ts');
