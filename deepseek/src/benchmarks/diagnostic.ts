...
async function measurePlaywrightInit() {
  ...
  const { initPlaywright, getDeepSeekAuth } = await import('../services/playwright.ts');
  await initPlaywright();
  const initTime = performance.now() - start;

  const headerStart = performance.now();
  await getDeepSeekAuth();
...
async function measureNetworkLatency() {
  ...
  const start = performance.now();
  await fetch(`${config.deepseek.baseUrl}/api/models`, {
...
async function runDiagnostic() {
  ...
  try {
    ...
    console.log(`\nNetwork latency to DeepSeek: ${net.networkLatencyMs}ms`);
  } catch (e: any) {
    console.log(`Network: SKIP (${e.message})`);
  }
  
  console.log('\n=== Likely Bottlenecks ===');
  console.log('1. getDeepSeekAuth() UI interactions: 2000-5000ms per call');
  console.log('2. Global mutex (accountMutex): serializes all chat requests');
