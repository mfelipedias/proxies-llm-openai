import { addAccount, removeAccount, listAccounts, getAccountCredentials, DeepSeekAccount } from './core/accounts.ts'
...
async function showMenu() {
  ...
  while (true) {
    ...
    clear()
    console.log('=== DeepSeekProxy Account Manager ===\n')
...
async function loginAllAccounts(browserType: BrowserType) {
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    try {
      const fullAccount: DeepSeekAccount = {
        id: creds.id,
        email: creds.email,
...
async function addAccountManualFlow(browserType: BrowserType) {
  ...
  console.log('A browser window will open. Please login to DeepSeek manually.')
