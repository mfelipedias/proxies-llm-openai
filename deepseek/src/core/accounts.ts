...
export function loadAccounts(): DeepSeekAccount[] {
  const db = getDatabase()
  const rows = db.prepare('SELECT id, email, password FROM accounts ORDER BY created_at ASC').all()
  return rows as DeepSeekAccount[]
}

export function addAccount(email: string, password: string, id?: string): DeepSeekAccount {
  ...
  const newAccount: DeepSeekAccount = {
...
export function listAccounts(): DeepSeekAccount[] {
  return loadAccounts().map(a => ({ id: a.id, email: a.email, password: '***' }))
}

export function getAccountCredentials(id: string): DeepSeekAccount | undefined {
  const db = getDatabase()
  const row = db.prepare('SELECT id, email, password FROM accounts WHERE id = ?').get(id)
  return row as DeepSeekAccount | undefined
}
