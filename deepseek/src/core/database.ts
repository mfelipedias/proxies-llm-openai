...
// Diretório de dados resolvido tardiamente (permite override por env nos testes).
function dataDir(): string {
  return process.env.DEEPSEEK_DATA_DIR
    ? path.resolve(process.env.DEEPSEEK_DATA_DIR)
    : path.resolve('data')
}
...
export function getDatabase(): Database.Database {
  ...
  const DB_PATH = path.join(DATA_DIR, 'deepseekproxy.db')
