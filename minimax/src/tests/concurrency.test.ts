  ...
  globalThis.fetch = async (input: any) => {
    ...
    if (url.includes('/api/models')) {
      return new Response(JSON.stringify({ 
        data: [{ id: 'minimax3.6-plus', owned_by: 'minimax', info: { created_at: Date.now(), meta: {} } }] 
      }), { status: 200 });
    }
...
test('No-thinking model variant is accepted', async () => {
  ...
  globalThis.fetch = async (input: any) => {
    ...
    if (url.includes('/api/models')) {
      return new Response(JSON.stringify({ 
        data: [{ id: 'minimax3.6-plus', owned_by: 'minimax', info: { created_at: Date.now(), meta: { max_context_length: 1000000 } } }] 
      }), { status: 200 });
    }
    if (url.includes('/api/v2/chat/completions')) {
      return new Response(
  ...
  try {
    ...
    const res = await app.fetch(
      new Request('http://localhost/v1/chat/completions', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          model: 'minimax3.6-plus-no-thinking',
