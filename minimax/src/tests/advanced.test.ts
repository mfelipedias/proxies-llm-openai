...
function setupFetchMock(handler: (url: string, init?: RequestInit) => Response | Promise<Response>) {
  ...
  globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (urlStr.includes('agent.minimax.io')) {
      // Handle models list request separately if handler doesn't
      if (urlStr.includes('/api/models')) {
         return new Response(JSON.stringify({ data: [{ id: 'minimax3.6-plus', owned_by: 'minimax' }] }), { status: 200 });
      }
...
test('multiturn-thinking-tools: maintains reasoning_content history', async () => {
  ...
  try {
    ...
    // The proxy transforms messages into a Minimax-compatible prompt.
...
test('streaming-whitespace: preserves exact whitespace', async () => {
  ...
  try {
    const req = new Request('http://localhost/v1/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: 'minimax3.6-plus', messages: [{role: 'user', content: 'test'}], stream: true })
    });
...
test('session-parent-tracking: appends messages using response message_id as parent', async () => {
  ...
  const restore = setupFetchMock((url, init) => {
    ...
    // Simulate Minimax returning a response_id
    const mockMessageId = capturedPayloads.length === 1 ? 'minimax-1001' : 'minimax-1002';
  ...
  try {
    ...
    // In Turn 2, parent_id should be minimax-1001 (the ID returned in Turn 1)
    assert.strictEqual(capturedPayloads[1].parent_id, 'minimax-1001', 'Turn 2 should use response_id from Turn 1 as parent');
