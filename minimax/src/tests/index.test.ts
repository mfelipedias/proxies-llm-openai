...
test('Models endpoint returns minimax3.6-plus and minimax3.6-plus-no-thinking', async () => {
  ...
  try {
    ...
    assert.ok(body.data.some((m: any) => m.id === 'minimax3.6-plus'));
    assert.ok(body.data.some((m: any) => m.id === 'minimax3.6-plus-no-thinking'));
  } finally {
    globalThis.fetch = originalFetch;
  }
});

test('Chat Completions endpoint with minimax3.6-plus (thinking enabled)', async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async (input: any) => {
    const url = typeof input === 'string' ? input : input.url;
    if (url.includes('/api/models')) {
...
test('Chat Completions returns explicit error for non-SSE upstream JSON errors', async () => {
  ...
  try {
    ...
    const body = await res.json();
    assert.match(body.error.message, /Minimax upstream error: RateLimited/);
...
test('Chat Completions endpoint - Non-streaming (stream: false)', async () => {
  ...
  try {
    ...
    const body = await res.json();
    assert.strictEqual(body.object, 'chat.completion');
    assert.strictEqual(body.model, 'minimax3.6-plus');
