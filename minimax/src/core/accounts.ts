...
export function loadAccounts(): MinimaxAccount[] {
  const db = getDatabase()
  const rows = db.prepare('SELECT id, email, password FROM accounts ORDER BY created_at ASC').all()
  return rows as MinimaxAccount[]
}

export function addAccount(email: string, password: string, id?: string): MinimaxAccount {
  ...
  const newAccount: MinimaxAccount = {
...
export function listAccounts(): MinimaxAccount[] {
  return loadAccounts().map(a => ({ id: a.id, email: a.email, password: '***' }))
}

export function getAccountCredentials(id: string): MinimaxAccount | undefined {
  const db = getDatabase()
  const row = db.prepare('SELECT id, email, password FROM accounts WHERE id = ?').get(id)
  return row as MinimaxAccount | undefined
}
