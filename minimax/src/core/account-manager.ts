import { MinimaxAccount, loadAccounts } from './accounts.ts'
...
// Conta sintética usada quando o DB está vazio: roteia para o perfil global
// (_default), onde o login manual foi feito. Permite rodar zero-config.
const GLOBAL_ACCOUNT: MinimaxAccount = { id: 'global', email: 'global', password: '' }

export function getNextAccount(): MinimaxAccount | null {
  ...
  let best: MinimaxAccount | null = null
...
export function getNextAvailableAccount(skipAccountId?: string): MinimaxAccount | null {
  const accounts = loadAccounts()
  if (accounts.length === 0) return skipAccountId === 'global' ? null : GLOBAL_ACCOUNT
