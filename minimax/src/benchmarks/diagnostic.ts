...
async function measureNetworkLatency() {
  ...
  const start = performance.now();
  await fetch(`${config.minimax.baseUrl}/api/models`, {
...
async function runDiagnostic() {
  ...
  try {
    ...
    console.log(`\nNetwork latency to Minimax: ${net.networkLatencyMs}ms`);
  } catch (e: any) {
    console.log(`Network: SKIP (${e.message})`);
  }
  
  console.log('\n=== Likely Bottlenecks ===');
  console.log('1. getMinimaxHeaders() UI interactions: 2000-5000ms per call');
  console.log('2. Global mutex (minimaxChatMutex): serializes all chat requests');
