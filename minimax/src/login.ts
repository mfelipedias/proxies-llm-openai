import { addAccount, removeAccount, listAccounts, getAccountCredentials, MinimaxAccount } from './core/accounts.ts'
...
async function showMenu() {
  ...
  while (true) {
    ...
    clear()
    console.log('=== MinimaxProxy Account Manager ===\n')
...
async function loginAllAccounts(browserType: BrowserType) {
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    try {
      const fullAccount: MinimaxAccount = {
        id: creds.id,
        email: creds.email,
...
async function addAccountManualFlow(browserType: BrowserType) {
  ...
  console.log('A browser window will open. Please login to Minimax manually.')
